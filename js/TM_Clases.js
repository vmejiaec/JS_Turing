...
TMachine.prototype.ejecUnaVez = function() {
  ...
  if (encontrado) {
    ...
    if (instrucciones[3] == "R" || instrucciones[3] == "r") pos++;
    else pos--;
    ...
    // Todo OK
    return true;
  } // En caso de que la máquina de turing no encuentre una línea qué ejecutar
  else this.parada = true;
  return false;
};
...
TMachine.prototype.cintaHtml = function() {
  ...
  var divFin = "</div>";
  var cintaHtml =
    this.cinta.substring(0, this.posCabeza) +
    divInicio +
    this.cinta.substring(this.posCabeza, this.posCabeza + 1) +
    divFin +
